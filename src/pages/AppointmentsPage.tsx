...
export function AppointmentsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Step 2: fetch patient profiles separately (avoids RLS join issues)
      ...
      if (mounted) {
        setAppointments(
          apptData.map(a => ({ ...a, patient: profileMap[a.patient_id] ?? null }))
        );
        setLoading(false);
      }
    })();
                  ...
                  <tr key={a.id} className="border-b border-purple/[0.04] last:border-0">
                    <td className="py-4 pr-4">
                      <PatientAvatar
                        ...
                        firstName={a.patient?.first_name ?? null}
                        lastName={a.patient?.last_name ?? null}
                      />
                      {a.patient?.phone && (
                        <p className="text-xs text-grey-500 mt-0.5 pl-11">{a.patient.phone}</p>
                      )}
                    </td>
